...
#define USE_C_FEISTEL_CODE  1     /* Set to 1 to use C code, 0 to inline via macro      */
...
#define LITTLE_ENDIAN 2 /* Intel platforms */
#define BIG_ENDIAN 3    /* Motorola and Sun among others */
...
/*
 *-------------------------------------------------------------------------------------------------
 * Copy a 16-byte buffer (word aligned) to another. Convert endianness if on little-
 * endian machine.
 *-------------------------------------------------------------------------------------------------
 */
#if ENDIAN_TYPE == LITTLE_ENDIAN
static void CopyConvertEndianness16
(
    VOLATILE const Word *src,
    VOLATILE Word *dst
)
{
    dst[0] = (LeftRotate(src[0], 8) & 0x00ff00ff) |
	(RightRotate(src[0], 8) & 0xff00ff00);
    dst[1] = (LeftRotate(src[1], 8) & 0x00ff00ff) |
	(RightRotate(src[1], 8) & 0xff00ff00);
    dst[2] = (LeftRotate(src[2], 8) & 0x00ff00ff) |
	(RightRotate(src[2], 8) & 0xff00ff00);
    dst[3] = (LeftRotate(src[3], 8) & 0x00ff00ff) |
	(RightRotate(src[3], 8) & 0xff00ff00);
}

#else
#define CopyConvertEndianness16(src, dst)  memcpy((char *)(dst), (char *)(src), 16)
#endif
...
/*
 *-------------------------------------------------------------------------------------------------
 * Transforms an array of 4 words by flipping the first two words with the last 2 words.
 *-------------------------------------------------------------------------------------------------
 */
#define SwapHalf(x)\
do {\
...
static void RotBlock
(
    VOLATILE const Word x[],
    const int n,
    Word y[]
)
{
...
static void Camellia_Feistel
(
    VOLATILE Word *x,
    VOLATILE const Word *k,
...
/* Routine derived from camellia.c
 *-------------------------------------------------------------------------------------------------
 * Module:	Camellia block cipher library.
 *
 * Description: Optimized, portable C implementation of the Camellia Cipher functions.
 *
 * Copyright (c) NTT, All Rights Reserved
 *-------------------------------------------------------------------------------------------------
 */
...
)
{
    ...
    if (keyBitLength == 128){
	CopyConvertEndianness16((Word *)rawKey, (Word *)t);
	for (i = 4; i < 8; i++) t[i] = 0;
    }
	...
	CopyConvertEndianness16((Word *)rawKey, (Word *)t);
	CopyConvertEndianness16(((Word *)rawKey)+4, ((Word *)t)+4);
    }
...
)
{
    ...
    VOLATILE Word *ct = (Word *)ciphertext;
    int flayerLimit = totalGrandRounds - 1;
    VOLATILE const Word *k = keyTable+4;

   /*
    *----------------------------------------------------------------------------------------------
    * Copy over plaintext to a ciphertext buffer, and change endianness if needed
    *----------------------------------------------------------------------------------------------
    */
    CopyConvertEndianness16((Word *)plaintext, ct);
    ...
    */
	...
	/* Camellia makes 6 feistel rounds */
	    for (j = 0; j < 6; j+=2, k += 4)
		Camellia_Feistel(ct,k,2);

	if (grandRounds < flayerLimit)
	{
/*
 *-------------------------------------------------------------------------------------------------
 * This is the same function as the diffusion function D of the accompanying documentation.
 * See section 3.2 of the accompanying documentation for properties of the FLlayer function.
 *----------------------------------------------------------------------------------------------
 */
	    ct[1] ^= LeftRotate(ct[0] & k[0], 1);
	    ct[0] ^= ct[1] | k[1];
	    ct[2] ^= ct[3] | k[3];
	    ct[3] ^= LeftRotate(ct[2] & k[2], 1);
	    k += 4;
	}
    }
...
)
{
    ...
    /*
    *----------------------------------------------------------------------------------------------
    * Copy over cipher text to a Word aligned buffer, and change endianness if needed
    *----------------------------------------------------------------------------------------------
    */

    CopyConvertEndianness16((Word *)ciphertext, pt);
 ...
 *----------------------------------------------------------------------------------------------
 */
	    pt[1] ^= LeftRotate(pt[0] & k[0], 1);
	    pt[0] ^= pt[1] | k[1];
	    pt[2] ^= pt[3] | k[-1];
	    pt[3] ^= LeftRotate(pt[2] & k[-2], 1);
	    k -= 4;
	}
    }
