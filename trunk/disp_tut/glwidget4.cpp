// Time-stamp: <glwidget4.cpp - 18:09:34 19.03.2006>
/*****************************************************************
...
GLuint GLWidget4::makeObject()
{
  ...
  double z[250][250];
  double L = 5, T = 5, x = 0, time = 0, accuracy = 245.0*Less1::perfomance;
  int i = 0, j = 0, N = Less3::N;
  double disp[N], spektrAmpl[N], spektrPhase[N];
  // Копируем статические компоненты в локальные (может скорость больше будет?)
  for (int i = 0; i < N/2; i++) {
    disp[i] = Less2::disp[i];
    spektrAmpl[i] = Less3::spektrAmpl[i];
    spektrPhase[i] = Less3::spektrPhase[i];
  }
